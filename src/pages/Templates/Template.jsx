...
export default function Template() {
  ...
  const loadTemplate = async () => {
    try {
      ...
      if (response.hasError) {
        alert(response.msg?.[0] || "Template not found.");
        navigate("/templates");
        return;
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        response = await templateService.update(id, form);
      } else {
        response = await templateService.save(form);
      }
      ...
      if (!response.hasError) {
        ...
        alert(response.msg?.[0] || "Something went wrong.");
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <MainLayout>
      ...
      <form onSubmit={handleSubmit}>
        <div className="mx-auto max-w-7xl rounded-3xl border border-slate-200 bg-white shadow-sm">
          {/* Header */}
          <div className="flex items-center justify-between border-b border-slate-200 p-6">
            <div>
              <h2 className="text-3xl font-bold text-slate-800">
